	...
	public function getData($id=0){
		...
		if($id!=0){
			...
			if($row){
				...
				$typeList = $roomDb->getRoomType($row['av_id']);
...
//		}
	...
	public function getLocationData($location_code=0){
		...
		$select = $db->select()
				->from(array('ar'=>$this->_name))
				->joinLeft(array('u'=>'tbl_user'),'u.iduser = ar.av_update_by', array('ar.av_update_by_name'=>'fName'))
	...
	public function getLocationVenue($location_code=0){
		...
		$select = $db->select()
				...
				->where('ar.av_location_code = '.$location_code)
				->where('ar.av_status = 1')
				->order('ar.av_room_name_short ASC');
	...
	public function getPaginateData($location_code = 0){
		...
		if($location_code!=0){
			$selectData = $db->select()
					->from(array('ar'=>$this->_name))
					->join(array('u'=>'tbl_user'),'u.iduser = ar.av_update_by', array('ar.av_update_by_name'=>'fName'))
					->where('ar.av_status = ?', 1)
					->where('ar.av_location_code = ?', $location_code)
					->order('ar.av_room_name ASC');
		}else{
			$selectData = $db->select()
					->from(array('ar'=>$this->_name))
					->join(array('u'=>'tbl_user'),'u.iduser = ar.av_update_by', array('al_update_by_name'=>'fName'))
					->where('ar.av_status = ?', 1)
					->order('ar.av_room_name ASC');	
		}
	...
	public function searchPaginate($location_id=0, $post = array()){
		...
		$selectData = $db ->select()
						...
						->join(array('u'=>'tbl_user'),'u.iduser = al.al_update_by', array('al_update_by_name'=>'fName'))
