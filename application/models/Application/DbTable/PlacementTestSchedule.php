...
class App_Model_Application_DbTable_PlacementTestSchedule extends Zend_Db_Table_Abstract {
	...
	public function search($post = array()){
		$db = Zend_Db_Table::getDefaultAdapter();
		$select = $db ->select()
						->from(array('aps'=>$this->_name))
						->join(array('al'=>'appl_location'), 'al.al_id = aps_location_id ')
             		    ->order('aps.aps_location_id ASC')
             		    ->order('aps.aps_test_date ASC');
             		    
        if( isset($post['location']) && $post['location']!="" ){
        	$select->where("aps.aps_location_id = '".$post['location']."'");
        } 
        
        if( (isset($post['dt_from']) && $post['dt_from']!="") && (isset($post['dt_to']) && $post['dt_to']!="")){
        	$select->where("aps.aps_test_date between '".date('Y-m-d', strtotime($post['dt_from']))."' and '".date('Y-m-d', strtotime($post['dt_to']))."'");
        }
