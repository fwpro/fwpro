...
class App_Model_Record_DbTable_AcademicYear extends Zend_Db_Table_Abstract
{
	...
	public function getCurrentAcademicYearData(){
		...
		$select = $db->select()
				->from($this->_name)
				->where('ay_end_date >= ?',date('Y-m-d') )
				->where('ay_start_date <= ?',date('Y-m-d') );
	...
	public function getAcademicYear($status=array()){

		
		$db = Zend_Db_Table::getDefaultAdapter();
		$select = $db->select()
				->from($this->_name)
				->where($this->_name.".ay_year_status in ('".implode("','",$status)."')")
				->order('ay_start_date');
